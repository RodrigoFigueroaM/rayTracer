    ...
    if objHit:
        ...
        if objHit.material == 'dielectric':
            ...
            if QVector3D.dotProduct(ray.d, normal) < 0:
                refracterRayDir = refract(ray.d, normal, 1.5)[1]
                ...
                isRayRefracted, refracterRayDir = refract(ray.d, -normal, 1/1.5)
                if isRayRefracted:
            ...
            color = hit(Ray(p, reflectionRefractionRayDirection), scene, 10000)[1]
            color2 = hit(Ray(p, refracterRayDir), scene, 10000)[1]
            print("1", color)
            print("2", color2)
            try:
                return R * color.color + (1 - R) * color2.color
            except:
                return R * QVector3D(0,0,0) + (1 - R) *  QVector3D(0,0,0)
