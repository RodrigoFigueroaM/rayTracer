                ...
                for p in range(0, self.antialiasing_level):
                    for q in range(0, self.antialiasing_level):
                        primary_ray = camera.ray_trace(x + (p + random()) / self.antialiasing_level,
                                                       ...
                                                       file.width)
                        pixel_color += self.__ray_hit_color(primary_ray, camera, scene, 0, t, dept=-5)
                pixel_color = PPMFile.clamp_pixel(
                    pixel=pixel_color / (self.antialiasing_level * self.antialiasing_level))
                file.write_QVector3D_to_file(pixel_color)
        file.close()

    def __ray_hit_color(self, ray, camera, scene, min_t, max_t, dept):
