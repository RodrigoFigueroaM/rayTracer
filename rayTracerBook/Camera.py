#! /usr/bin/env python
from PyQt5.QtGui import QVector3D


class Camera:
    """docstring for Camera"""
    def __init__(self, position=QVector3D(0, 0, 3),
                 lookAt=QVector3D(0, 0, 0),
                 up=QVector3D(0, 1, 0),
                 viewingPlaneDistance = 1):
        super(Camera, self).__init__()
        self._pos = position
        self._lookAt = lookAt
        self._viewingPlaneDistance = viewingPlaneDistance
        #compute uvw
        self._dir = (lookAt - self._pos).normalized()
        self._back = (self._pos - lookAt).normalized()
        self._right = QVector3D.crossProduct(up, self._back) # change order?
        self._up = QVector3D.crossProduct(self._back, self._right)
        self._center = self._pos + self.direction

    @property
    def position(self):
        return self._pos

    @position.setter
    def position(self, position):
        self._pos = position

    @property
    def direction(self):
        self._dir = self._lookAt - self._pos
        return self._dir.normalized()

    @property
    def lookAt(self):
        return self._lookAt

    @lookAt.setter
    def lookAt(self, lookat):
        self._lookAt = lookat

    @property
    def up(self):
        return self._up.normalized()

    @property
    def center(self):
        self._center = self.position + self.direction
        return self._center

    @property
    def right(self):
        return self._right

    @property
    def viewingPlaneDistance(self):
        return self._viewingPlaneDistance

    @viewingPlaneDistance.setter
    def viewingPlaneDistance(self, viewingPlaneDistance):
        self._viewingPlaneDistance = viewingPlaneDistance

    class Ray:
        """generate a ray = e + td"""

        def __init__(self, origin, direction):
            self.e = origin
            self.d = direction

        def __str__(self):
            return "e = {}\nd = {}".format(self.e, self.d)

    def orthoRayCast(self, i, j, width, height):
        l = (0 - width / 2.0 + 0.5)
        r = (width - width / 2.0 + 0.5)
        b = (0 - height / 2.0 + 0.5)
        t = (500 - height / 2.0 + 0.5)
        u = l + (r - l) * (i + 0.5) / (r - l)
        v = b + (t - b) * (j + 0.5) / (t - b)
        rayOrigin = self._pos + u * self._right + v * self._up
        rayDirection = self._dir
        return Camera.Ray(rayOrigin, rayDirection.normalized())

    def perspectiveRayCast(self,  i, j, width, height):
        l = (0 - width / 2.0 + 0.5)
        r = (width - width / 2.0 + 0.5)
        b = (0 - height / 2.0 + 0.5)
        t = (height - height / 2.0 + 0.5)
        u = l + (r - l) * (i + 0.5) / (r - l)
        v = b + (t - b) * (j + 0.5) / (t - b)
        rayDirection = (- self._viewingPlaneDistance * self._back + u * self._right + v * self._up) - self._pos
        return Camera.Ray(self._pos, rayDirection.normalized())

    def obliqueRayCast(self, i, j, width, height):
        l = (0 - width / 2.0 + 0.5)
        r = (width - width / 2.0 + 0.5)
        b = (0 - height / 2.0 + 0.5)
        t = (height - height / 2.0 + 0.5)
        u = l + (r - l) * (i + 0.5) / (r - l)
        v = b + (t - b) * (j + 0.5) / (t - b)
        rayDirection = u * self._right + v * self._up - self._back * self._viewingPlaneDistance
        return Camera.Ray(self._pos, rayDirection.normalized())

    def rayTrace(self, x, y, width, height, s=1.0):
        y = height - y
        xw = s * (x - 0.5 * (width + 1.0))
        yw = s * (y - 0.5 * (height + 1.0))
        s = xw * self._right + yw * self._up - self._back * self._viewingPlaneDistance
        return Camera.Ray(self.position, (s - self._pos).normalized())
